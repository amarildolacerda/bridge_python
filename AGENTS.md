...
8. Service name UDP discovery (`"esp-matter-bridge"` vs `"esp-rmaker-gateway"`) precisa ser idêntico no cliente e no gateway, senão o discovery nunca responde.
...
15. Arquivos duplicados em `/project/main/` e `/project/rainmaker/main/` — sempre manter em sincronia (copiar ambos ao alterar).```