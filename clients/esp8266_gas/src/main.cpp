...
static const char *TAG = "esp8266-gas";
...
static bool wifi_setup(bool force_config_portal = false)
{
    ...
    if (wifiManager.startConfigPortal("ESP8266_Gas", "password123"))
    {
...
static void read_sensor(void)
{
    ...
    s_gas_level = map(raw, 0, 1024, 0, 100);
    s_gas_level = constrain(s_gas_level, 0, 100);
    ...
    else if (s_gas_level < GAS_ALERT_THRESHOLD)
    {
        digitalWrite(NORMAL_PIN, HIGH);
        s_normal_state = true;
    }
...
void setup(void)
{
    ...
    Serial.printf("\n");
    Serial.printf("============================================\n");
    Serial.printf("  ESP8266 Gas Detector v1.0\n");
...
void loop(void)
{
...
#ifdef NORMAL_PIN
    if (!s_sensor_error && s_gas_level < GAS_ALERT_THRESHOLD && s_normal_state)
    {
        if (now - s_last_state_update >= NORMAL_PULSE_MS)
        {
            digitalWrite(NORMAL_PIN, LOW);
            s_normal_state = false;
        }
    ...
    {
        if (now - last_alarm >= ALARM_BLINK_FAST_MS)
        {
            last_alarm = now;
    ...
    {
        if (now - last_alarm >= ALARM_BLINK_SLOW_MS)
        {
            last_alarm = now;
            digitalWrite(ALARM_PIN, !digitalRead(ALARM_PIN));
        }
