...
static const char *TAG = "esp8266-occupancy";
...
static bool s_occupancy = false;
static bool s_last_pir = LOW;
static unsigned long s_pir_high_start = 0;
static unsigned long s_pir_low_start = 0;
static unsigned long s_last_detection_ms = 0;
...
static void send_state(bool force)
{
    ...
    bool changed = force || (s_occupancy != last_sent);
    if (!changed)
        return;
    ...
    if (http_post("/api/device/state", body))
    {
        ...
        last_sent = s_occupancy;
        Serial.printf("[%s] occupancy=%s\n", TAG, s_occupancy ? "true" : "false");
    }
}
...
static bool wifi_setup(bool force_config_portal = false)
{
    ...
    if (wifiManager.startConfigPortal("ESP8266_Presenca", "password123"))
    {
...
static void read_sensor(void)
{
    ...
    if (pir == HIGH && s_last_pir == LOW)
    {
        s_pir_high_start = now;
        s_occupancy = true;
        s_last_detection_ms = now;
        Serial.printf("[%s] Motion detected\n", TAG);
    }

    if (pir == LOW && s_last_pir == HIGH)
    {
        s_pir_low_start = now;
    }

    if (s_occupancy && pir == LOW && (now - s_pir_low_start >= OCCUPANCY_DEBOUNCE_MS))
    {
        s_occupancy = false;
        Serial.printf("[%s] Motion cleared (debounce %dms)\n", TAG, OCCUPANCY_DEBOUNCE_MS);
    }
...
static void handle_api_state(void)
{
    String json;
    {
        ...
        if (s_last_detection_ms) doc["last_detection_s"] = (millis() - s_last_detection_ms) / 1000;
...
void setup(void)
{
    ...
    Serial.printf("\n");
    Serial.printf("============================================\n");
    Serial.printf("  ESP8266 Presence Sensor v1.0\n");
    Serial.printf("  FW      : " FW_VERSION "\n");
